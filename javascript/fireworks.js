...
function draw() {
    ...
    if (random(1) < 0.02){
        fireballs.push(new Particle(random(0, winWidth), winHeight, 3, false, random(255)));
    ...
    for (let i=fireballs.length-1; i>= 0; i--){
        if (fireballs[i].velocity.y >= 0){
            for (let j=0; j<200;j++){
                debris.push(new Particle(fireballs[i].position.x, fireballs[i].position.y, 1, true, random(255)));
